...
static struct dvobj_priv	*pci_dvobj_init(struct pci_dev *pdev, const struct pci_device_id *pdid)
{
	...
	if (pdev->bus->number == 0x00)
		dvobj->mask_addr =
			(unsigned long)ioremap(PCIE_SLOT1_MASK, 0x100);
	else if (pdev->bus->number == 0x01)
		dvobj->mask_addr =
			(unsigned long)ioremap(PCIE_SLOT2_MASK, 0x100);

	if (dvobj->mask_addr == 0) {
		RTW_ERR("Can't map TRAN/MASK mem\n");
		goto release_regions;
	}

	dvobj->tran_addr = dvobj->mask_addr + PCIE_TRANSLATE_OFFSET;
...
#ifdef RTK_129X_PLATFORM
	if (status != _SUCCESS && dvobj->mask_addr != 0) {
		pci_iounmap(pdev, (void *)dvobj->mask_addr);
		dvobj->mask_addr = 0;
	}
#endif
